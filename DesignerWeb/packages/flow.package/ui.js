...
COMPONENT('designer', function() {
	...
	self.add = function(item) {
		...
		for (var i = 0; i < input; i++) {
			var o = points.asvg('circle').attr('class', 'input').attrd('index', i).attr('cx', width/2).attr('cy', 0).attr('r', radius);
			if (inputcolors)
		...
		for (var i = 0; i < output; i++) {
			var o = points.asvg('circle').attr('class', 'output').attrd('index', i).attr('cx', width/2).attr('cy', height).attr('r', radius);
			if (outputcolors)
